...
public class BookingServiceImpl implements BookingService {
    ...
    // MÃƒÂ©todos delegados para cÃƒÂ¡lculo dinÃƒÂ¡mico
    ...
    public PurchaseResponseDTO processPurchase(PurchaseRequestDTO request, User currentUser) {
        ...
        if (redisIdempKey != null) {
            ...
            if (existingVal != null) {
                if ("PROCESSING".equals(existingVal)) {
                    throw new BusinessRuleException("TransacciÃƒÂ³n en curso, por favor espere.");
                } else {
                    try {
            ...
            if (Boolean.FALSE.equals(isFirst)) {
                throw new BusinessRuleException("TransacciÃƒÂ³n en curso concurrente, por favor espere.");
            }
        }

        try {
            // 1. VALIDACIÃƒâ€œN INICIAL: Ã‚Â¿QuÃƒÂ© estÃƒÂ¡ comprando?
        ...
        if (!hasTickets && !hasSnacks) {
            throw new BusinessRuleException("El carrito estÃƒÂ¡ vacÃƒÂ­o. Debe seleccionar entradas o snacks.");
        }

        if (hasTickets && request.funcionId() == null) {
            throw new BusinessRuleException("Debe especificar una funciÃƒÂ³n para la compra de entradas.");
        }
        ...
        if (request.funcionId() != null) {
            showtime = showtimeRepository.findById(request.funcionId())
                    .orElseThrow(() -> new ResourceNotFoundException("FunciÃƒÂ³n no encontrada"));
        ...
        // ==========================================
        // LÃƒÂ³gica de Venta en Taquilla delegada
        // ==========================================
        User buyerUser = taquillaService.resolveBuyerUser(currentUser, request.clienteId());
        // Re-adjuntar a la sesiÃƒÂ³n de Hibernate actual para evitar LazyInitializationException
        if (buyerUser != null && buyerUser.getId() != null) {
            buyerUser = userRepository.findById(buyerUser.getId()).orElse(buyerUser);
        }

        // 2. VALIDACIÃƒâ€œN DE CANTIDAD DE ENTRADAS
        if (hasTickets && request.asientos().size() > 10) {
            throw new BusinessRuleException("No puedes comprar mÃƒÂ¡s de 10 entradas por transacciÃƒÂ³n.");
        }

        // 3. VALIDACIÃƒâ€œN DE PRECIOS Y REDIS (ENTRADAS Y SNACKS)
        ...
        if (hasTickets) {
            for (var seatReq : request.asientos()) {
                ...
                if (lockOwner == null || !lockOwner.equals(currentUser.getId().toString())) {
                    throw new BusinessRuleException("El tiempo de reserva expirÃƒÂ³ o el asiento no te pertenece.");
                }
                ...
                if (seatReq.pendingBenefitId() != null) {
                    ...
                    if (pendingBen != null && pendingBen.getEstado() == com.cinezone.demo.model.enums.BenefitStatus.DISPONIBLE) {
                        ...
                        if (pendingBen.getTipoEntrada() == com.cinezone.demo.model.enums.TipoEntrada.GENERAL_2D && "VIP".equals(salaTipo)) {
                            throw new BusinessRuleException("El beneficio no es vÃƒÂ¡lido para salas VIP.");
                        }
                    } else {
                        throw new BusinessRuleException("El beneficio de cumpleaÃƒÂ±os ya no estÃƒÂ¡ disponible.");
                    }
                } else {
        ...
        // 4. VALIDACIÃƒâ€œN DEL MONTO TOTAL (Usamos el calculado por el servidor para seguridad)
        if (request.montoTotalPago().subtract(expectedTotal).abs().compareTo(new BigDecimal("0.1")) > 0) {
            ...
            // throw new BusinessRuleException("El monto total enviado no coincide con el cÃƒÂ¡lculo del servidor.");
        }
        
        BigDecimal finalTotal = expectedTotal; // Priorizamos el cÃƒÂ¡lculo del servidor

        // 4.5. VALIDACIÃƒâ€œN PESIMISTA DE LÃƒÂMITE MENSUAL DE BENEFICIOS
        if (hasTickets) {
                    ...
                    if (ben != null && ben.getFormato() != null && !ben.getFormato().equals(com.cinezone.demo.util.AppConstants.FORMATO_TODOS)) {
                        ...
                        if (!match) {
                            throw new com.cinezone.demo.exception.BenefitFormatMismatchException(
                                "El beneficio '" + ben.getName() + "' solo es vÃƒÂ¡lido para formato " + benFmt + " pero la funciÃƒÂ³n es " + showFmt
                            );
                        }
                    }
                }
        ...
        // 8. YA NO SE ACTUALIZAN PUNTOS AQUÃƒÂ (Se harÃƒÂ¡ en confirmPurchase cuando se pague)
        
        // 9. GENERAR QR Y RESPUESTA
        String infoCine = (showtime != null) ? showtime.getMovie().getTitulo() : "SOLO DULCERÃƒÂA";
    ...
    public void lockSeat(com.cinezone.demo.dto.LockSeatRequestDTO request, User currentUser) {
        ...
        // Intentar setear la llave solo si no existe, con expiraciÃƒÂ³n de 5 minutos (300 segundos)
        ...
        if (Boolean.FALSE.equals(success)) {
            // Verificar si el asiento ya estÃƒÂ¡ bloqueado por el mismo usuario (renovar expiraciÃƒÂ³n)
            String lockOwner = (String) redisTemplate.opsForValue().get(redisKey);
            if (lockOwner != null && lockOwner.equals(currentUser.getId().toString())) {
    ...
    public java.util.List<java.util.Map<String, Object>> getTicketTypes(Long showtimeId, User currentUser) {
        ...
        for (TicketBasePrice base : basePrices) {
            ...
            // Verificar si estÃƒÂ¡ desactivada explÃƒÂ­citamente para esta sede
            ...
            // Filtrar por Formato de ProyecciÃƒÂ³n
    ...
    public void confirmPurchase(java.util.UUID bookingId) {
        ...
        if (currentUser.getRol() != com.cinezone.demo.model.enums.Role.CLIENT) {
            return; // Los empleados de taquilla/admin no acumulan puntos por ventas anÃƒÂ³nimas o directas
        }
        ...
        if (puntosCalculados > 0) {
            pointHistoryRepository.save(PointHistory.builder()
                    ...
                    .descripcion(hasTickets ? "Compra de entradas/snacks" : "Compra solo dulcerÃƒÂ­a")
                    .booking(booking)
                    .build());
        }
        ...
        if (booking.getUser() != null
            ...
            if (puntosOtorgados != null && !puntosOtorgados.isEmpty()) {
                ...
                if (puntosADescontar > 0) {
                    ...
                    // Registrar historial de deducciÃƒÂ³n de puntos por cancelaciÃƒÂ³n
                    PointHistory descuento = PointHistory.builder()
                            .user(freshUser)
                            .puntos(-puntosADescontar)
                            .tipo(PointType.CANJEADO)
                            .descripcion("Descuento por anulaciÃƒÂ³n de boleta " + booking.getCodigoUnico() + " | " + motivo)
                            .booking(booking)
                            .build();
    ...
    public void cleanupAbandonedBookings() {
        // Limpiar reservas pendientes de mÃƒÂ¡s de 5 minutos
        ...
        for (Booking booking : abandonedBookings) {
            try {
                ...
                this.cancelBooking(booking.getId(), booking.getUser(), "Tiempo lÃƒÂ­mite de compra expirado (Auto-CancelaciÃƒÂ³n)");
            } catch (Exception e) {
