...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    @org.springframework.data.jpa.repository.Query("SELECT t FROM Ticket t WHERE t.booking.showtime.id = :showtimeId AND t.booking.estado IN (com.cinezone.demo.model.enums.BookingStatus.VALIDA, com.cinezone.demo.model.enums.BookingStatus.USADA, com.cinezone.demo.model.enums.BookingStatus.PENDIENTE)")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT b.showtime.movie.titulo, COUNT(t) FROM Ticket t " +
           "JOIN t.booking b " +
           "WHERE b.estado IN (com.cinezone.demo.model.enums.BookingStatus.VALIDA, com.cinezone.demo.model.enums.BookingStatus.USADA, com.cinezone.demo.model.enums.BookingStatus.PENDIENTE) AND (:sedeId IS NULL OR b.showtime.cinema.id = :sedeId) " +
    ...
    @org.springframework.data.jpa.repository.Query("SELECT s.movie.titulo, " +
           "COALESCE( (SELECT COUNT(t) FROM Ticket t JOIN t.booking b WHERE b.showtime = s AND b.estado IN (com.cinezone.demo.model.enums.BookingStatus.VALIDA, com.cinezone.demo.model.enums.BookingStatus.USADA, com.cinezone.demo.model.enums.BookingStatus.PENDIENTE) AND b.fechaCompra >= :startDate AND b.fechaCompra <= :endDate), 0), " +
    ...
    @org.springframework.data.jpa.repository.Query("SELECT s.movie.titulo, SUM(t.precioPagado) FROM Ticket t JOIN t.booking b JOIN b.showtime s " +
           "WHERE b.estado IN (com.cinezone.demo.model.enums.BookingStatus.VALIDA, com.cinezone.demo.model.enums.BookingStatus.USADA, com.cinezone.demo.model.enums.BookingStatus.PENDIENTE) AND (:sedeId IS NULL OR s.cinema.id = :sedeId) " +
           "AND b.fechaCompra >= :startDate AND b.fechaCompra < :endDate " +
