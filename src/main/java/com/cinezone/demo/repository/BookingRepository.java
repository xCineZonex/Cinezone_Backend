...
public interface BookingRepository extends JpaRepository<Booking, UUID> {
    ...
    @Query("SELECT SUM(b.montoTotal) FROM Booking b WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra BETWEEN :start AND :end")
    BigDecimal calculateTotalSalesAll(
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end
    );

    @Query("SELECT SUM(b.montoTotal) FROM Booking b LEFT JOIN b.showtime s LEFT JOIN s.cinema c WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra BETWEEN :start AND :end AND c.id = :sedeId")
    ...
    @Query("SELECT COUNT(b) FROM Booking b WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra BETWEEN :start AND :end")
    Long countBookingsAll(
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end
    );

    @Query("SELECT COUNT(b) FROM Booking b LEFT JOIN b.showtime s LEFT JOIN s.cinema c WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra BETWEEN :start AND :end AND c.id = :sedeId")
    ...
    @Query("SELECT SUM(b.montoTotal) FROM Booking b WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE')")
    BigDecimal calculateTotalRevenue();

    @Query("SELECT SUM(b.montoTotal) FROM Booking b LEFT JOIN b.showtime s LEFT JOIN s.cinema c WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND c.id = :sedeId")
    ...
    @Query("SELECT CAST(b.fechaCompra AS LocalDate), SUM(b.montoTotal) FROM Booking b " +
           "WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra >= :since " +
    ...
    @Query("SELECT CAST(b.fechaCompra AS LocalDate), SUM(b.montoTotal) FROM Booking b " +
           ...
           "WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra >= :since AND c.id = :sedeId " +
           "GROUP BY CAST(b.fechaCompra AS LocalDate) ORDER BY CAST(b.fechaCompra AS LocalDate) ASC")
    ...
    @Query("SELECT s.movie.titulo, SUM(b.montoTotal) FROM Booking b " +
           "LEFT JOIN b.showtime s " +
           "WHERE b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra BETWEEN :start AND :end " +
    ...
    List<Booking> findByUser_IdOrderByFechaCompraDesc(UUID userId);
    ...
    @Query("SELECT SUM(b.montoTotal) FROM Booking b WHERE b.employee.id = :employeeId AND b.estado IN ('VALIDA', 'USADA', 'PENDIENTE') AND b.fechaCompra >= :start")
    BigDecimal sumTotalByEmployeeAndDate(@Param("employeeId") UUID employeeId, @Param("start") LocalDateTime start);

    List<Booking> findByEmployee_IdOrderByFechaCompraDesc(UUID employeeId);
}
